// app/(dashboard)/email/_components/email-content-wrapper.tsx
"use client";

// NOTE: Removed EmailDecryptor as decryption is now SERVER-SIDE and AUTOMATIC.

import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Download, File as FileIcon, Image as ImageIcon, Lock, Video as VideoIcon } from "lucide-react";
import { useEffect, useState } from 'react'; // Added useEffect for simulated server fetch
...
// ====================================================================
// ATTACHMENT UTILITIES (KEPT AS IS)
// ====================================================================
...
// ====================================================================
// EMAIL CONTENT WRAPPER (UPDATED)
// ====================================================================

interface EmailContentWrapperProps {
    email: any; 
    // This prop now holds the FINAL subject/body, pre-processed by the server.
    // The server component must call autoDecryptAction and pass the result here.
    finalSubject: string; 
    ...
    // NEW: Optional prop to indicate decryption status/error from the server
...
export function EmailContentWrapper({ 
    ...
    printLogoUrl, 
    isRead,
    decryptionError, // Check for server-side decryption errors
}: EmailContentWrapperProps) { 
    
    // Determine the actual sender/recipient details for the header (unchanged logic)
    const toRecipients = email.recipients.filter((r: any) => r.type === "TO");
    const ccRecipients = email.recipients.filter((r: any) => r.type === "CC");

    // RENDER: Decryption Error Message (if failed server-side)
    ...
    // RENDER: Normal/Decrypted Content
    return (
        <div 
            className="flex-1 flex flex-col h-full w-full min-w-0 bg-white dark:bg-[#0f172a] relative overflow-hidden 
            print:fixed print:inset-0 print:z-50 print:bg-white print:h-auto print:overflow-visible print:block"
        >
            {/* Read Listener (must be client side) */}
            {!email.isSender && (
                <EmailReadListener emailId={email.id} isRead={isRead} />
            )}
                ...
                <>
                    ...
                    <div className="mb-8">
                        <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white mb-6 leading-tight print:text-black print:text-2xl print:mb-4 flex items-center gap-3">
                            {/* Display Lock icon if it was originally E2EE and successfully decrypted */}
                            {email.isE2EE && (
                                ...
                                <div className="flex flex-col gap-1">
                                    ...
                                    <div className="text-sm text-slate-600 dark:text-slate-400 print:text-slate-800">
                                        ...
                                        {toRecipients.length > 0
                                            ? toRecipients
                                                .map((r: any) => r.user.name || r.user.email)
                                                .join(", ")
                    ...
                    <div
                        ...
                        // Use the FINAL body HTML passed from the Server Component
