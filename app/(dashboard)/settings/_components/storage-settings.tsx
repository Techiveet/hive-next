...
// Define the expected client-side shape to match the initial state structure
...
export function StorageSettings() {
  ...
  const [form, setForm] = useState<ClientStorageSettings>({
    provider: "LOCAL",
    accessKeyId: "",
    secretAccessKey: "",
    bucket: "",
  ...
  useEffect(() => {
    startTransition(async () => {
      ...
      // ✅ FIX: Safely map the returned Prisma data (which has 'string | null')
      // to the component state (which requires 'string').
      if (data) {
        setForm((prev) => ({
          ...prev,
          ...data,
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Choose where system files and backups are stored.
        </CardDescription>
      </CardHeader>
          ...
          <div className="space-y-4 border p-4 rounded-md bg-muted/10">
            ...
            <div className="space-y-2">
              ...
              <div className="text-[11px] text-muted-foreground mb-1">
                Go to Google Cloud Console {">"} IAM & Admin {">"} Service
                Accounts {">"} Create Key (JSON). Paste the entire content here.
              </div>
              <Textarea
                className="font-mono text-xs min-h-[150px]"
                value={form.gdriveJson || ""}
                onChange={(e) =>
                  setForm({ ...form, gdriveJson: e.target.value })
                }
            ...
            <div className="flex items-start gap-2 bg-yellow-50 text-yellow-800 p-3 rounded text-xs border border-yellow-200">
              ...
              <div>
                <strong>Important:</strong> Enable the "Google Drive API" in
                your Google Cloud project. Files will be stored in a folder
                named "Hive Backups" in the Service Account's drive. To see
                files, you must share that folder with your personal email
                later.
              </div>
            </div>
          </div>
