...
async function authorizePermAction(
  ...
  if (tenantId) {
    ...
    const central = await prisma.userRole.findFirst({
      where: {
        userId: user.id,
        tenantId: null,
...
export async function upsertPermissionAction(rawData: unknown) {
  ...
  if (input.id) {
    ...
    // Tenant cannot modify system/global permission
