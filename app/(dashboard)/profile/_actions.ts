...
export async function changePasswordAction(currentPass: string, newPass: string) {
  ...
  const existingHash = credentialAccount?.password ?? dbUser.password ?? null;
  ...
  if (credentialAccount) {
    ...
    // No account with password yet: create a 'credential' account like in users-actions
...
export async function verifyPasswordAction(password: string) {
  ...
  // 1. Try finding password on User model
  const dbUser = await prisma.user.findUnique({ 
    where: { id: user.id },
    include: { accounts: true } // Include accounts to check there too
  });
  ...
  // 2. If no password on User, check the Credential Account
  if (!hashedPassword && dbUser.accounts.length > 0) {
    const account = dbUser.accounts.find(a => a.password);
    if (account) {
      hashedPassword = account.password;
    }
  }

  if (!hashedPassword) return false; // No password set
